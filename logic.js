...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if(mode === 'review') {
        document.getElementById('intro-title').innerText = "Chế độ xem lại bài đã làm";
        document.querySelector('.rules-box').style.display = 'none';
        document.querySelector('button[onclick="startExam()"]').innerText = "Xem Chi Tiết Bài Làm";
...
function startExam() {
    ...
    if(mode === 'review') {
        ...
        document.querySelector('button[onclick="confirmSubmit()"]').style.display = 'none';
        
        let resultId = localStorage.getItem('reviewResultId');
        let myResult = DATA.results.find(r => r.id === resultId);
        
        if(myResult) {
            restoreAnswers(myResult.userAnswers);
            processSubmit(false); 
        }
        return;
    }
...
function restoreAnswers(savedAnswers) {
    if(savedAnswers.p1) {
        savedAnswers.p1.forEach((val, i) => {
            if(val !== null) {
                let r = document.querySelector(`input[name="p1-q${i}"][value="${val}"]`);
                if(r) { r.checked = true; r.closest('.option-label').classList.add('selected'); }
            }
        });
    }
    if(savedAnswers.p2) {
        savedAnswers.p2.forEach((grp, i) => {
            grp.forEach((val, sIdx) => {
                if(val !== null) {
                    let r = document.querySelector(`input[name="p2-q${i}-s${sIdx}"][value="${val}"]`);
                    if(r) r.checked = true;
                }
            });
        });
    ...
    if(savedAnswers.p3) {
        savedAnswers.p3.forEach((val, i) => {
            if(val !== null) document.getElementById(`in-p3-q${i}`).value = val;
        });
    }
}

async function processSubmit(isSaving = true) {
    clearInterval(timerInterval);
    ...
    if(currentExamData.part1) {
        currentExamData.part1.forEach((q, i) => {
            ...
            let userVal = sel ? parseInt(sel.value) : null;
            userAnswers.p1.push(userVal);
            ...
            if (userVal === q.ans) {
                ...
                let uAns = sel ? labels[userVal] : "Chưa chọn";
    ...
    if(currentExamData.part2) {
        currentExamData.part2.forEach((q, i) => {
            ...
            q.stmts.forEach((stmt, sIdx) => {
                ...
                let userVal = sel ? sel.value : null;
                grpAns.push(userVal);

                let isCorrect = userVal === stmt.ans;
                if(isCorrect) correctInGrp++;
                
                let iconStr = isCorrect ? '<span style="color:var(--success)">✓</span>' : '<span style="color:var(--danger)">✗</span>';
                expHtml += `<li style="padding-left:0; list-style:none;">${iconStr} <strong>Ý ${subLabels[sIdx]}:</strong> (Bạn: ${userVal || '-'} | Chuẩn: ${stmt.ans}) - ${stmt.exp}</li>`;
                document.getElementsByName(`p2-q${i}-s${sIdx}`).forEach(el => el.disabled = true);
            });
            userAnswers.p2.push(grpAns);
    ...
    if(currentExamData.part3) {
        currentExamData.part3.forEach((q, i) => {
            ...
            let uVal = input.value.trim();
            userAnswers.p3.push(uVal || null);

            let compU = uVal.replace(',', '.'); 
            let compA = q.ans.replace(',', '.');

            if (compU !== '' && compU === compA) {
    ...
    if(isSaving) {
        ...
        if(user) {
            ...
            await saveToGitlab(`User ${user.name} submitted exam ${examId} with score ${finalScore}`);
        }
    }
}
