...
const examId = urlParams.get('exam') || 'de1'; // Mặc định
...
// Khi trang load xong mới bắt đầu đọc dữ liệu
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Tính tổng số item phải làm (Số câu P1 + Số Ý P2 + Số câu P3)
...
function startExam() {
    if(!originalExamData) {
        alert("Dữ liệu lỗi. Vui lòng F5!"); return;
    }
    ...
    timerInterval = setInterval(() => {
        ...
        // HẾT GIỜ: Tự động thu bài bất chấp làm xong hay chưa
        if (timeRemaining <= 0) { 
...
// THUẬT TOÁN KIỂM TRA ĐIỀU KIỆN NỘP BÀI
function confirmSubmit() {
    // Nếu số câu đã làm nhỏ hơn tổng số câu => Bật cảnh báo, từ chối nộp
    ...
    // Nếu đã làm đủ 100% => Hiện bảng xác nhận nộp
...
function processSubmit() {
    ...
    // Xây dựng Dashboard thống kê (ẩn đi môn nào không có phần đó)
    let statHtml = '';
    let maxPossScore = 0;
    if(currentExamData.part1) {
        let maxP1 = currentExamData.part1.length * 0.25;
        maxPossScore += maxP1;
        statHtml += `<div class="stat-box st-p1"><div style="color: #64748b; font-size: 0.9rem;">Phần I (Max ${maxP1})</div><div style="font-size: 1.5rem; font-weight: bold; color: #3b82f6;">${scoreP1.toFixed(2)}</div></div>`;
    }
    if(currentExamData.part2) {
        let maxP2 = currentExamData.part2.length * 1.0;
        maxPossScore += maxP2;
        statHtml += `<div class="stat-box st-p2"><div style="color: #64748b; font-size: 0.9rem;">Phần II (Max ${maxP2})</div><div style="font-size: 1.5rem; font-weight: bold; color: #8b5cf6;">${scoreP2.toFixed(2)}</div></div>`;
    }
    if(currentExamData.part3 && currentExamData.part3.length > 0) {
        let maxP3 = currentExamData.part3.length * 0.25;
        maxPossScore += maxP3;
        statHtml += `<div class="stat-box st-p3"><div style="color: #64748b; font-size: 0.9rem;">Phần III (Max ${maxP3})</div><div style="font-size: 1.5rem; font-weight: bold; color: #ec4899;">${scoreP3.toFixed(2)}</div></div>`;
    }
    document.getElementById('stat-grid').innerHTML = statHtml;

    // Chuyển trang và đổ dữ liệu
