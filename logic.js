...
const examId = urlParams.get('exam') || 'de1'; // Mặc định lấy de1
...
document.addEventListener("DOMContentLoaded", () => {
    // Kéo dữ liệu từ window (do de1.js hoặc de2.js cung cấp)
    originalExamData = window[examId]; 

    // Kiểm tra nếu dữ liệu bị lỗi hoặc chưa tải được
    if(!originalExamData) {
        document.getElementById("intro-title").innerText = "Lỗi: Không tìm thấy dữ liệu đề thi (Vui lòng kiểm tra file " + examId + ".js)";
        document.getElementById("header-subject-info").innerText = "Lỗi tải dữ liệu";
        return;
    }
    
    // Nếu có dữ liệu thì cập nhật giao diện
...
function startExam() {
    if(!originalExamData) {
        alert("Lỗi: Dữ liệu chưa tải xong. Vui lòng F5 tải lại trang!");
        return;
    }
