...
async function saveToGitlab(actionMsg) {
    ...
    try {
        ...
        if(res.ok) {
            console.log("Đã lưu thành công lên GitLab!");
            return true;
        } else {
            const errorData = await res.json();
            alert("LỖI LƯU LÊN GITLAB: " + (errorData.message || "Kiểm tra lại quyền của Token!"));
            return false;
        }
    } catch (err) { 
        alert("Lỗi mạng khi kết nối GitLab!"); 
        return false;
    }
}
...
async function register() {
    ...
    // Đảm bảo kéo dữ liệu mới nhất trước khi kiểm tra trùng email
    ...
    // Gắn biến kiểm tra xem có lưu lên Gitlab thành công hay không
    ...
    if(isSaved) {
        ...
        // Nếu lỗi, xóa user vừa tạo ảo đi
        DATA.users.pop();
    }
}
...
async function login() {
    ...
    // Đã xóa hàm await loadGitlabData() ở đây để tránh lỗi delay của API.
    // Dữ liệu đã được nạp sẵn khi bạn load trang hoặc sau khi đăng ký thành công.
